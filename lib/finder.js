...
function findRestaurant(req) {
  ...
  let cacheKey = [location, radius, category, search].join(':');
  ...
  let searchOptions = {
    location: location,
    sort: YELP_SORT_BEST_MATCH,
    category_filter: category ? category : YELP_CATEGORY_RESTAURANTS,
    radius_filter: radius
  };

  if (search) {
    searchOptions.term = search;
  }
